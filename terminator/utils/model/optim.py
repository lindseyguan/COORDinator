...
def get_std_opt(parameters, d_model, lr_multiplier=1, regularization=1e-3, state=None, finetune=False, finetune_lr=1e-6, warmup=4000):
    if finetune:
        optim = torch.optim.Adam(parameters, lr=finetune_lr, weight_decay=regularization)
    else:
        optim = NoamOpt(d_model, lr_multiplier, warmup,
