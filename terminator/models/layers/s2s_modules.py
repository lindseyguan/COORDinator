...
class NodeMPNNLayer(nn.Module):
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        ...
        dh = self.dense(h_V)
        if mask_attend is not None:
            dh = mask_attend[:,:,0].unsqueeze(-1) * dh
        h_V = self.norm[1](h_V + self.dropout(dh))
...
class EdgeMPNNLayer(nn.Module):
    ...
    def forward(self, h_E, h_EV, E_idx, merge_fn, inv_mapping=None, mask_E=None, mask_attend=None):
        """ Parallel computation of full transformer layer """

        dh = self.W3(F.relu(self.W2(F.relu(self.W1(h_EV)))))
        dh = merge_fn(dh, E_idx, inv_mapping)  # does this help?
        if mask_attend is not None:
